...
# C array visitor
class ArrayAccessVisitor(c_ast.NodeVisitor):
    def __init__(self):
        self.found = False
...
def check_ex1():
    try:
        ...
        if v.count == 0 and r.count == 0:
            return (round(grade), utils.failed('[vector_set] don\'t forget to call free+malloc or use realloc...'), '')
        else:
            grade += 5
...
def check_ex2():
    try:
        ...
        # compile
        task = utils.make(target='bubble')
        if task.returncode != 0:
            return (round(grade), utils.failed('compilation error'), task.stderr.decode().strip())
        
        # check for free calls
        v = ArrayAccessVisitor()
        ast = utils.parse_c('ex2/bubble')
        v.reset()
        
        v.visit(utils.find_func(ast, 'bubbleSort'))
        if v.found:
            return (round(grade), utils.failed('use pointers instead of array accesses in bubbleSort!'), '')
        
        v.visit(utils.find_func(ast, 'swap'))
        if v.found:
            return (round(grade), utils.failed('use pointers instead of array accesses in swap!'))
        
        # run tests
        task = utils.execute(cmd=['./bubble'], timeout=5)
        if task.returncode != 0:
            return (round(grade), utils.failed('runtime error'), task.stderr.decode().strip())
        ...
        for (exp, out) in zip(expected, output):
            ...
            if (len(out) == 0) or (out[:8] == "Original"):
                continue
...
def lab3_struct():
    not_found = utils.expected_files([ './ex1/vector.c', './ex2/bubble.c' ])
    if len(not_found) == 0:
        table = []
        ...
        table.append(('2. bubble', *ex2_result[0:2]))

        errors = ''
        errors += '\n' + utils.create_error('vector', ex1_result[2])
        errors += '\n' + utils.create_error('bubble', ex2_result[2])
        errors = errors.strip()
